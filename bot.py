...
def main_menu():
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="📚 Обучение операционным процессам")],
...
# ✅ Тест завершен
async def finish_test(message: types.Message, state: FSMContext, correct_answers: int, questions: list):
    ...
    # Получаем номер телефона из state
    data = await state.get_data()
    user_phone = data.get("phone_number", "Неизвестен")
    ...
    admin_text = (
        ...
        f"🎯 Баллы: {correct_answers} из {len(questions)}"
    )
    await bot.send_message(chat_id=1889888021, text=admin_text)
